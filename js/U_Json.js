		...
		if (item[i].ZipName_ === BID || BID === "BALL") {
			if (item[i].FileNo_.match($('#search').val()) !== null || 
				...
				item[i].Lng_.match($('#search').val()) !== null){
					var dv = '<div class="card col-md-4"> <div class="card-body"> <h5 class="card-title">' + item[i].ZipName_ + '</h5><p class="card-text">' + item[i].Save_Date_ + '</p> <p class="card-text">' + item[i].BeforeDesc_ + '</p> <p class="card-text">' + item[i].address_ + '</p> <a href="#" class="btn btn-primary">' + item[i].UnitName_ + '</a> </div> </div>'
			$("#R_space").append(dv);
			}
		}
	}
